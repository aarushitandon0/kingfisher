  ...
  return (
    <div className="min-h-0 flex-1 overflow-y-auto px-4 py-4 md:px-6">
      <div className="max-w-6xl">
        <h1 className="t-title">Validation</h1>
        ...
        {res.data && doc && (
          <>
            ...
            <div className="mt-3 flex flex-wrap items-center gap-4 t-ui">
              <span className="flex items-center gap-1">
                <span className="text-muted">Fold</span>
                {Object.keys(doc.folds).map((f) => (
                  <button key={f} type="button" aria-pressed={fold === f} onClick={() => setFold(f)} className={`px-1.5 ${fold === f ? "text-ink underline underline-offset-4" : "text-muted"}`}>
                    {f}
                    {doc.splits?.[f] && <span className="t-value-sm text-muted"> (trained to {doc.splits[f].train_end})</span>}
                  </button>
                ))}
              </span>
              <span className="flex items-center gap-1">
                ...
                {VARS.map((v) => (
                  <button key={v} type="button" aria-pressed={variable === v} onClick={() => setVariable(v)} className={`px-1.5 ${variable === v ? "text-ink underline underline-offset-4" : "text-muted"}`}>
                    {VARIABLE_LABEL[v]}
                  </button>
                ))}
              </span>
...
function H({ children }: { children: React.ReactNode }) {
  return <h2 className="t-title mt-8 mb-1 border-b border-hairline pb-1">{children}</h2>;
}
...
function Reliability({ vm }: { vm: VariableMetrics }) {
  ...
  return (
    <section aria-label="Reliability diagram">
      <h2 className="t-title mb-1">Reliability</h2>
...
function Observability({ vm }: { vm: VariableMetrics }) {
  ...
  return (
    <section>
      <h2 className="t-title mb-1">Observable vs driver-only</h2>
...
function LeadTime({ doc, vm }: { doc: MetricsDocument; vm: VariableMetrics }) {
  ...
  return (
    <section className="mt-6">
      <h2 className="t-title mb-1">Lead time</h2>
      {note ? (
        <p className="t-ui">
